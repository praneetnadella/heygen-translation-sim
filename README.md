...
- **Polling Mechanism** - Implemented an adaptive polling mechanism to reduce unnecessary calls to the server. There is the option to either autoPoll by default or manually poll for the status of the job.
...
- **Estimated Time Remaining** - Add a way to get the estimated time remaining for a job to start/complete. This would be more straightforward when actual timings are added to the videos and an estimated time for each job is calculated. The implementation right now is just based of a random delay.
- **Concurrency** - Currently the implementation assumes the server is using all its resources to translate a video. However, depending on the server, and project parameters, it might be better/asked of us to have multiple jobs running at once. This would require a more complex server/client interaction, and would also require a more complex polling mechanism. But it could lead to huge gains in the efficiency of the server. Especially if tied with the "speed server" idea, where the speed server has many threads to crank out "low impact" jobs quickly.
